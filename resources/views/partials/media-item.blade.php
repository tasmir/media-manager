    ...
    <div class="aspect-square bg-slate-100 overflow-hidden relative">
        ...
        <div
            ...
            @if(!request()->exists('trashed'))
                ...
                @can('files_delete')
                    <form action="{{ route($page_date['loop']->delete, $media) }}" method="POST"
            ...
            @else
                ...
                @can('files_delete')
                    <form action="{{ route('admin.files.restore', $media->id) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit"
                                class="p-2 bg-white rounded-lg text-slate-700 hover:bg-emerald-600 hover:text-white transition-colors duration-200 shadow-lg"
                                title="Restore">
                    ...
                    <form action="{{ route('admin.files.force-delete', $media->id) }}" method="POST"
                          class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" 
                                class="p-2 bg-white rounded-lg text-slate-700 hover:bg-rose-600 hover:text-white transition-colors duration-200 shadow-lg"
                                onclick="return confirm('Are you sure you want to delete this media? {{ request()->exists('trashed') ? 'This action acts firmly.' : '' }}')"
                                title="{{ request()->exists('trashed') ? 'Delete Permanently' : 'Delete' }}">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                      d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                            </svg>
